        ...
        # for repeat in range (1):
            ...
            with mirrored_strategy.scope():  
                ...
                autoencoder.compile(optimizer = opt,
                        ...
                        jit_compile=False,
                        metrics = ['mean_absolute_error', 'mean_squared_error', 'binary_crossentropy'])

                plot_model(autoencoder, to_file = 'autoencoder.png', show_shapes=True, expand_nested=True)
            ...
            autoencoder.save('saved_model/f_ar_o2_image_spectra_autoencoder_ConvNext-vT_repeat_' + str(repeat) + '_mse_bn_latent='+str(latent_units)+'_batch_size='+str(batch_size)+'_initial_lr='+str(initial_learning_rate)+'latent_reg='+str(reg_latent)+'latent_norm='+latent_norm)
            encoder.save('saved_model/f_ar_o2_image_spectra_encoder_ConvNext-vT_repeat_' + str(repeat) + '_mse_bn_latent='+str(latent_units)+'_batch_size='+str(batch_size)+'_initial_lr='+str(initial_learning_rate)+'latent_reg='+str(reg_latent)+'latent_norm='+latent_norm)
